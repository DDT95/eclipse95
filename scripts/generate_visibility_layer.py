...
BOUNDARY = ROOT / "public/data/val-doise.geojson"
OUT_IMAGE = ROOT / "public/data/visibility-relief-2026-08-12.png"
OUT_META = ROOT / "public/data/visibility-relief-2026-08-12.json"
CACHE = ROOT / ".cache/terrarium-z12"
ZOOM = 12
BEARING = 284.0
SUN_ALTITUDE = 7.9
GRID_WIDTH = 320
...
def main() -> None:
    ...
    ring = np.asarray(geojson["features"][0]["geometry"]["coordinates"][0], dtype=float)
    west, south = ring.min(axis=0)
    east, north = ring.max(axis=0)
    ...
    distances = np.geomspace(0.08, 20.0, 34)
    memory: dict = {}
    ...
    meta = {
        ...
        "resolution": "~30 m DEM; grille de restitution ~250–350 m",
        "scope": "Relief uniquement; bâtiments et végétation vérifiés au clic"
    }
